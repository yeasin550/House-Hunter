  ...
  const handleSearchHouse = async (event) => {
    ...
    try {
      ...
      const response = await fetch(
        "https://house-hunter-server-pi.vercel.app/searchHouses",
        {
          method: "POST",
          headers: {
    ...
    <div>
      <div id="header" className="hero mt-3 rounded-lg overflow-hidden">
        ...
        <div className="hero-content text-center text-neutral-content">
          <form
            onSubmit={handleSearchHouse}
            className="max-w-[800px] min-h-[400px] flex flex-col items-center justify-center"
          >
            <h1 className="text-[30px] text-white font-medium mb-1">
            ...
            <div className="w-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-2 justify-between">
              <div className="bg-white flex items-center rounded-lg">
                <input
                  ...
                  className="p-3 rounded-lg border-none border-0 outline-0 w-full text-[var(--primary-color)] text-[15px] font-medium"
                />
              </div>
              ...
              <div>
                <select
                  name="residential"
                  className="select select-bordered w-full text-[var(--primary-color)] text-[15px] font-medium"
                >
              ...
              <div className="bg-white flex items-center rounded-lg col-span-1 md:col-span-2">
                <input
                  ...
                  className="p-3 rounded-lg border-none border-0 outline-0 w-full text-[var(--primary-color)] text-[15px] font-medium"
                />
                <span className="pr-2 text-[var(--primary-color)]">
            ...
            <div className="w-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-2 justify-between pt-[8px]">
              ...
              <button
                type="submit"
                className="py-[10px] mt-4 md:mt-0 md:py-0 bg-[var(--primary-color)] flex items-center justify-center text-[18px] rounded-lg text-white"
              >
                Find House
              </button>
            </div>
      ...
      {searchHouse.length > 0 && (
        <>
          <div className="my-5" data-aos="fade-up">
            <h1 className="mb-3 text-[30px] font-semibold text-[var(--primary-color)] text-center border-b-[3px] w-fit mx-auto">
              Your Current Search:{" "}
            </h1>
